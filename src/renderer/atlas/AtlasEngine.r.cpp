...
void AtlasEngine::_updateConstantBuffer() const noexcept
{
    ...
    data.underlinePos.x = _r.fontMetrics.underlinePos;
    data.underlinePos.y = _r.fontMetrics.underlinePos + _r.fontMetrics.lineThickness;
    data.strikethroughPos.x = _r.fontMetrics.strikethroughPos;
    data.strikethroughPos.y = _r.fontMetrics.strikethroughPos + _r.fontMetrics.lineThickness;
...
void AtlasEngine::_drawGlyph(const AtlasQueueItem& item) const
{
    ...
    // !!! IMPORTANT !!!
    // D2D1_DRAW_TEXT_OPTIONS_NO_SNAP snaps the baseline to whole pixels, which is something we technically
    // want (it makes text look crisp), but fails in weird ways if `scalingRequired` is true (see below).
    // As our scaling matrix's dx/dy (center point) is based on the coordinates stored in the `cells`
    // array each cell we draw gets a unique, fractional baseline which gets rounded differently.
    // This causes wide glyphs (cellCount > 1) to potentially get tiles that are vertically misaligned by a pixel.
    // The resulting text rendering looks especially bad for ligatures like "====" in Cascadia Code,
    // where each "=" might be vertically misaligned by a pixel (same for any box drawings).
    auto options = D2D1_DRAW_TEXT_OPTIONS_NO_SNAP;
    ...
    else
    {
        ...
        {
            scalingRequired = true;
            scale = _r.fontMetrics.scale;
            actualSize.x *= scale.x;
            actualSize.y *= scale.y;
        }
